...
packGrid = true
dir = "../big_output/"
t_prefix = dir*"gamma80"
h_prefix = dir*"gamma80"
...
init_solu = solve(init_ense, SOSRI(), EnsembleDistributed(), trajectories=nTraj, dt=1e-4, adaptive=false)
for i in 1:nTraj
...
for j in 1:nOuts-1
    ...
    solu = solve(ense, SOSRI(), EnsembleDistributed(), trajectories=nTraj, callback=cb_set, dt=1e-4, adaptive=false)
    for i in 1:nTraj
