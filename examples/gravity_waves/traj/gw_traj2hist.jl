...
#grid parameters
const nx = Int(round(Lx/dx))
const ny = 1
const nz = 2 + Int(round(Lz/dz))
const vol = dx*dy*dz
const XG = dx .* collect(0:nx-1/2)
const XC = XG .+ dx/2
const YG = dy .* collect(0:ny-1/2)
const YC = YG .+ dy/2
const RF = dz .* collect(1:-1:-nz+1) #length nz+1
const RC = (RF[1:end-1] .+ RF[2:end]) ./ 2
const nOuts = Int(floor(tStop/wFreq) + 1)

#histogram bin edges and flips
const edges = (vcat(XG, maximum(XG)+dx), reverse(RF))
