...
v = np.abs(p['TRAC01']).max().values/10
print(v)
...
def initialize_plots():
    ...
    cmaps = [gen_op_cmap(plt.cm.get_cmap('tab10')(i+1)) for i in range(nTracs)]
    ...
    try:
        ...
        for i in range(nTracs-1,-1,-1):
            p_p[i] = a_p.pcolormesh(p['XC'], p['YC'], p['TRAC0'+str(i+1)].isel(time=0),
                                    cmap='RdBu_r', shading='gouraud', vmin=-v, vmax=v,  animated=True)
        ...
        if writeMovie:
            ...
            today = np.datetime64('today').item()
            mname = '../figures/{0:02d}{1:02d}_diff_movie.mp4'.format(today.month, today.day)
