...
for ds_j, j1 in zip(ds.values(), range(nConfs)):
    ...
    for j2 in range(nSnaps):
        tmp = ds_j['TRAC{0:02d}'.format(3*j2+1)].interp(XC=r*np.cos(θ), YC=r*np.sin(θ))
...
def initialize_plots():
    ...
    #declare plots
    fTot = plt.figure(figsize=(10, 7), constrained_layout=True)
    ...
    for a in aVar:
        a.set_xlabel('time')
        a.set_ylabel(r'${\bf E}[R^2]$')
    #aSnp[-1,0].set_xlabel('$x$')
    #aSnp[-1,0].set_ylabel('$y$')
    ...
    #titles
    aVar[0].set_title('(a) centered release')
    aVar[1].set_title('(b) far-field release')
    for j in range(nConfs):
        aSnp[0,2*j].set_title('initial', fontsize='small')
        aSnp[0,2*j+1].set_title('final', fontsize='small')
    ...
    pVar = [[None for _ in range(nPerCf+2)] for j in range(2)]
    pSnp = [[[None for _ in range(2)]
             for j2 in range(nSnaps)]
             for j1 in range(nConfs)]
...
def Σp_lines_small(t, j):
    ...
    for Σ0_j in Σ0:
        pVar[j][-1] = aVar[j].plot(t, Σp_small(t, j, Σ0_j), 
...
def Σp_lines_large(t, j):
    ...
    for Σ0_j in Σ0:
        pVar[j][-1] = aVar[j].plot(t, Σp_large(t, j, Σ0_j),
...
def tidy_up_plots():
    #legends
    labels0 = [
        'full Monte Carlo', 'reduced Monte Carlo', 'reduced Dedalus',
        'reduced MITgcm', 'deterministic case', r'$R \ll a$ limit'
    ]
    labels1 = [r'$R \gg a$ limit']
    aVar[0].legend(pVar[0], labels0,
                   title='Simulation', fontsize='small', title_fontsize='small',
                   loc=4, handlelength=1.0)
    aVar[1].legend([pVar[1][-1]], labels1,
    ...
    for j1 in range(nConfs):
        ...
        aSnp[-1,2*j1].set_yticklabels(
            ...
            position=(np.pi,0), fontsize='x-small', ha='center'
        ) 
    ...
    if saveFigures:
        ...
        plt.savefig('../figures/kv_fig2_x.png', dpi=200, transparent=False)
    ...
    try:
        ...
        for j in range(1, nPerCf+1): 
            pVar[0][j-1], = aVar[0].plot(
                t0, ds[dirs[0]]['Σo{0:02d}'.format(j)]
            )
            pVar[1][j-1], = aVar[1].plot(
                t1, ds[dirs[1]]['Σo{0:02d}'.format(j)]
            )
        Σp_lines_small(ds[dirs[0]]['time'].values, 0)
        Σp_lines_large(ds[dirs[1]]['time'].values, 1)

        #deterministic case at reviewer's suggestion
        pVar[0][-2], = aVar[0].plot(t0, ds[dirs[0]]['Σo05'], 'k:', lw=1)
        pVar[1][-2], = aVar[1].plot(t1, ds[dirs[1]]['Σo05'], 'k:', lw=1)
